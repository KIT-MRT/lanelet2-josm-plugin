...
- **Private undo, not JOSM Commands.** The Jython never touched the global
  `UndoRedoHandler`; deletes use `NotesUndoStack`. A `Command` would attach
  to whichever map layer is active, so Ctrl+Z after creating a note would
  undo the wrong dataset.
